...
class CreateAccount:
	...
	def is_invalid_email_message(self):
		...
		if msg == 'Invalid email address.': return True
		return False

	def is_already_registered_message(self):
		msg = self.browser.find_element(*self.INVALID_EMAIL_MESSAGE).text
		if 'An account using this email address has already been registered.' in msg: return True
		return False
...
class SignIn:
	...
	INVALID_MESSAGE = (By.CSS_SELECTOR, '#center_column > div.alert.alert-danger > ol > li')
	...
	def is_invalid_error_message_displayed(self):
		...
		if 'Authentication failed' in msg: return True
		return False
