...
class CreateAccountInfo:
	...
	def is_account_creation_form_available(self):
		...
		if form.is_displayed: return True
		return False
	...
	def is_required_firstname_error_message_displayed(self):
		...
		if 'firstname is required' in msg: return True
		return False

	def is_required_lastname_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'lastname is required' in msg: return True
		return False

	def is_required_password_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'passwd is required' in msg: return True
		return False

	def is_invalid_password_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'passwd is invalid' in msg: return True
		return False

	def is_required_address_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'address1 is required' in msg: return True
		return False

	def is_required_city_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'city is required' in msg: return True
		return False

	def is_required_state_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'This country requires you to choose a State' in msg: return True
		return False

	def is_zipcode_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if "The Zip/Postal code you've entered is invalid. It must follow this format: 00000" in msg: return True
		return False

	def is_required_country_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'Country is invalid' in msg: return True
		return False
		
	def is_required_mobilephone_error_message_displayed(self):
		msg = self.browser.find_element(*self.ERROR_MESSAGE).text
		if 'You must register at least one phone number' in msg: return True
		return False
